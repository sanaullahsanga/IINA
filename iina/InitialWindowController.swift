...
class InitialWindowController: NSWindowController {
  ...
  @IBAction func clickedOnResume(_ sender: Any) {
    let delegate=NSApplication.shared.delegate as! AppDelegate
    guard !delegate.option.urls.isEmpty else { return }
    let urlsss: URL = delegate.option.urls[0]
    if urlsss.absoluteString=="" {
      _="nothing"
    }
    else{
      delegate.exportmovie.isHidden=false
    }
  }
  ...
  func loadLastPlaybackInfo() {
    if Preference.bool(for: .recordRecentFiles),
      ...
      let delegate=NSApplication.shared.delegate as! AppDelegate
      var aray:[String]=[]
      aray.append(lastFile.absoluteString)
      delegate.option.urls=aray.compactMap{URL(string: $0)}
      guard !delegate.option.urls.isEmpty else { return }
      let urlsss: URL = delegate.option.urls[0]
      if urlsss.absoluteString=="" {
        _="nothing"
...
extension InitialWindowController: NSTableViewDelegate, NSTableViewDataSource {
  ...
  func tableViewSelectionDidChange(_ notification: Notification) {
    ...
    let delegate=NSApplication.shared.delegate as! AppDelegate
    var aray:[String]=[]
    aray.append(url.absoluteString)
    delegate.option.urls=aray.compactMap{URL(string: $0)}
    guard !delegate.option.urls.isEmpty else { return }
    let urlsss: URL = delegate.option.urls[0]
    if urlsss.absoluteString=="" {
