...
class AppDelegate: NSObject, NSApplicationDelegate {
  @IBOutlet weak var exportmovie: NSMenuItem!
  ...
  var option: QuickTimeOptionsWindowsController = {
    let option = QuickTimeOptionsWindowsController(windowNibName: "QuickTimeOption")
    return option
  }()
  var outputsetingcontroller: OutputModuleSettingsController = {
    let outputsetingcontroller = OutputModuleSettingsController()
    return outputsetingcontroller
  }()
  ...
  @IBAction func openFile(_ sender: AnyObject) {
    ...
    if panel.runModal() == .OK {
      ...
      if panel.urls.count > 0 {
        option.urls = panel.urls
        
        //outputsetingcontroller.urls = panel.urls
    ...
    guard !option.urls.isEmpty else { return }
    let url: URL = option.urls[0]
    if url.absoluteString=="" {
      _="nothing"
    }
    else{
      exportmovie.isHidden=false
    }
  }
  ...
  @IBAction func transcodeVideo(_ sender: NSMenuItem) {
    /*Logger.log("Menu - Open file to transcode video")
    let panel = NSOpenPanel()
    panel.title = NSLocalizedString("alert.choose_media_file.title", comment: "Choose Media File")
    panel.canCreateDirectories = false
    panel.canChooseFiles = true
  ...
  @IBAction func openVideoConvertion(_ sender: NSObject) {
    guard !option.urls.isEmpty else { return }
    let url: URL = option.urls[0]
    if url.absoluteString=="" {
      _="nothing"
      ...
      outputsetingcontroller.showWindow(self)
    }
  }
